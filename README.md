...
持仓截图也可以交给 Agent 识别，再通过 `import_holdings` 批量写入本地文件；面板会自动刷新。在个股详情页点「AI 分析」即可生成完整解读，报告以格式化 Markdown 在独立浮层中呈现（生成中显示加载动画），缓存到本地并支持重新生成——结果直接展示在浮层，无需回看对话。插件只修改本地持仓数据，不执行真实交易。
...
- **数据来源开关（来源级）**：`公开数据` / `妙想数据` / `盈米` 各自独立开关、**即时同步模型工具**。关闭「公开数据」→ 模型的公开行情/K线等工具（约 23 个）被注销、按钮显示当前工具数；关闭妙想/盈米 → 其桥接工具热重载注销。可只用某一来源（如只开妙想）或混用。
- **公开数据明细（子项级）**：按 capability 选择使用哪些 provider（`quote`/`kline`/`hk_*` 等常多源共存）：多选、点击顺序即优先级、绿点=探测可用（作为数据获取偏好）。

选择保存到本地 `data/provider-policy.json`、`data/mcp-sources-state.json`。面板数据始终走完整 provider（探测/默认顺序），不受来源开关影响。
